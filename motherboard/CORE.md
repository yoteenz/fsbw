...
- **Cart / Wishlist:** Stored in localStorage (global and per-user keys). Synced to backend on sign-in/sync where applicable. **BCF bundle-deal** lines (`bcfBundleDeal`): in the **shopping bag** they use the same controls as **A/C** booking lines — **QTY** label + **×** remove only (no **+ LIST**, no ± stepper, no **SAVE FOR LATER**). If the user **no longer qualifies** for premium-gated features (`isPremiumMemberForGatedFeatures` in `premiumMemberAccess.ts` — active premium subscription and/or **BLACK** spend tier), bundle-deal rows are **removed** from **`cartItems`** and **`savedForLater`** on load and on **`signInStateChanged`** / window **`focus`** (`applyStripIneligibleBcfBundleDealsToAllStoredCarts` in **`App.tsx`**); **`CartDropdown`**, **`/checkout`**, **`/checkout/summary`** (localStorage fallback), bag **`loadCartItems` / `loadSavedForLater`** also strip when reading storage. **`MOVE TO BAG`** from saved ignores bundle-deal lines when not premium.
