...
- **Account → Rewards (premium / Stripe):** The subscription upgrade chart, tier **SELECT** buttons, **CONFIRM SUBSCRIPTION**, and **Subscribe with card (Stripe)** are on **Account → Rewards** at **`/account/rewards`** (`src/pages/account/membership/page.tsx`). There is no separate nav item labeled “Membership”; older docs saying “Account → Membership” mean this Rewards page. Stripe CTA shows only when **`GET /api/stripe/membership-available`** returns **`available: true`** (server has secret key + three price IDs) and the user has a **Supabase** session (Bearer from `getAccessToken()`), not local-only sign-in alone.
