...
/**
 * GET /api/orders — read own orders JSONB.
 * PUT is disabled: orders are appended by Stripe webhooks / service role only.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'PUT') {
    return res.status(403).json({
      error: 'Order writes are disabled for clients.',
      hint: 'Orders are created after Stripe payment_intent.succeeded (webhook). Sync with GET /api/orders.',
    });
  }
