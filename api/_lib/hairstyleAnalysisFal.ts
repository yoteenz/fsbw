...
async function resolveClientPreviewBuffer(raw: string, siteOrigin: string): Promise<Buffer> {
  ...
  const normalized = await normalizeClientPreviewBuffer(buf, mimeFromUrl(publicUrl));
  return normalized.buf;
}
...
export async function generateHairstyleAnalysisWithFal(
  ...
  const composited = await compositeHairstyleAnalysisPostProcess({
    ...
    restoreFreeClientPanel:
      analysis.tier === 'free' && clientPreviewPreEdited && identityBuf.length > 0,
