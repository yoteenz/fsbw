...
export async function getLatestPsaThread(userId: string): Promise<PsaThreadRow | null> {
  const supabase = getSupabaseAdminServiceRole();
  const { data, error } = await supabase
    .from('psa_threads')
    .select('*')
...
export async function listPsaThreads(userId: string, limit = THREAD_LIST_LIMIT): Promise<PsaThreadSummary[]> {
  const supabase = getSupabaseAdminServiceRole();
...
export async function resolvePsaThreadForChat(input: {
  ...
  const latest = await getLatestPsaThread(input.userId);
