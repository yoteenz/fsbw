...
import {
  bawSalonRightPartFrontDonorSideViewBlock,
  bawSalonRightPartHairEditLockBlock,
  bawSalonRightPartMiddleFrontDonorBlock,
  bawSalonRightPartPlacementRefPromptBlock,
  bawSalonRightPartSideViewBodyLockBlock,
  bawSalonRightPartZeroPriorityBlock,
} from './bawSalonPartPlacementRefs.js';
  ...
  const partBlock =
    partSelection === 'MIDDLE'
      ...
      : partSelection === 'LEFT'
        ...
        : '**UI R (RIGHT part):** **Not** UI L. Part **image LEFT** (**left third**). **Comb-over:** part + **image LEFT** shoulder hair on the **same side** — bulk from **behind** over that shoulder, **NOT** UI L top-forward cascade. **FORBIDDEN:** part **image RIGHT** (UI **L**).';
...
/**
 * **UI R part FRONT:** `[ placement guide, MIDDLE-part FRONT ]` — IMAGE 1 = part groove authority.
 */
function buildBawSalonRightPartFromMiddleFrontPrompt(
  ...
  const styleKeep =
    salon === 'layers'
      ? 'Keep the **same uniform voluminous layered S-waves** (not ringlets), volume, length, and color from **IMAGE 2** — **only** the **part groove** comes from **IMAGE 1**.'
      : salon === 'crimps'
        ? 'Keep the **same crimp texture, scale, length, and color** from **IMAGE 2** — **only** the **part groove** comes from **IMAGE 1**.'
        : 'Keep the **same bone-straight flat-ironed** finish, length, and color** from **IMAGE 2** — **only** the **part groove** comes from **IMAGE 1**.';
  const bangsLine = options?.includeBangs ? curtainBangsAddonForSalonPart('RIGHT') : null;

  return [
    bawSalonRightPartZeroPriorityBlock(1),
    'You get **2 images in order**.',
    bawSalonRightPartPlacementRefPromptBlock(1),
    bawSalonRightPartMiddleFrontDonorBlock(2, salonLabel),
    '**TASK:** **UI R / RIGHT part** FRONT view. Part groove = **IMAGE 1** (mirror of **UI L** — groove **image LEFT**, **not image RIGHT**). Color/texture/length = **IMAGE 2**. Apply **UI R comb-over drape** (part + **image LEFT** shoulder same side; hair from **behind**). **FORBIDDEN:** **UI L** groove (**image RIGHT**); **FORBIDDEN:** same groove position as LEFT part.',
    salonPartMustOverrideInputReferenceBlock('RIGHT'),
    salonPartDirectionSemanticsBlock(),
    bawSalonModeLockBlock(salon),
    styleKeep,
    bawSalonDrapeBlockForPart('RIGHT'),
    ...(bangsLine ? [bangsLine] : []),
    bawSalonRightPartHairEditLockBlock(),
    BAW_GPT2_LOGO_AND_HAIR_ONLY_LOCK,
    'Output must be extremely high-quality, crisp and pixel-perfect.',
...
/**
 * **UI R part L/R cameras:** `[ gray-brick side, placement guide, FRONT donor ]` — body=IMAGE 1, part=IMAGE 2, texture=IMAGE 3.
 */
function buildBawSalonRightPartSideViewWithPlacementRefPrompt(
  angle: 'left' | 'right',
  salon: 'layers' | 'crimps' | 'flat_iron',
  catalog: CatalogColorForLayersPrompt,
  options?: { includeBangs?: boolean }
  ...
  return [
    bawSalonRightPartZeroPriorityBlock(2),
    'You get **3 images in order**.',
    bawSalonRightPartSideViewBodyLockBlock(angle),
    bawSalonRightPartPlacementRefPromptBlock(2),
    bawSalonRightPartFrontDonorSideViewBlock(3, salonLabel, hex, catalog.label),
    '**TASK:** Composite onto **IMAGE 1** **' +
      angleLabel +
      '** body: **part groove from IMAGE 2** (UI R = **image LEFT** — mirror of UI L); **curl/color/length from IMAGE 3** only. **FORBIDDEN:** **UI L** part (**image RIGHT** groove); **FORBIDDEN:** same part side as LEFT part; **FORBIDDEN:** copying both-shoulder hair from IMAGE 2.',
    bawSalonModeLockBlock(salon),
    ...
    'Composite: **IMAGE 1** exact **' +
      angleLabel +
      '** body + **IMAGE 2** exact **UI R** part groove + **IMAGE 3** hair texture/color (**' +
      salonLabel +
      '**).',
  ].join(' ');
}
...
export function buildBawSalonSidePartFromMiddleFrontPrompt(
  ...
  if (targetPart === 'RIGHT' && options?.hasRightPartPlacementRef) {
    return buildBawSalonRightPartFromMiddleFrontPrompt(salon, options);
  }
  const salonLabel = bawSalonModePromptLabel(salon);
  const partTask =
    targetPart === 'LEFT'
      ? '**UI L / LEFT part**: part groove **image RIGHT** (**right third** of forehead) — **forward sweep** to **viewer’s LEFT** shoulder. **Not** UI R.'
      : '**UI R / RIGHT part**: part groove **image LEFT** (**left third** of forehead) — **same side** as **image LEFT** shoulder. **Comb-over** from **behind** — **not** UI L top-sweep. **Not** UI L (part **image RIGHT**).';
  const drapeChange =
    targetPart === 'RIGHT'
      ? ' Apply **UI R comb-over drape** (part + shoulder **same side**, hair from **behind** on shoulder).'
      : ' Apply **UI L forward-sweep drape** (part **image RIGHT**, cascade **image LEFT** shoulder).';
  const styleKeep =
    salon === 'layers'
  ...
  return [
    ...
    '**TASK:** **Recreate this FRONT photograph** with **only** the **part** and **part-specific drape** changed to ' +
      partTask +
      drapeChange +
      ' **Do not** mirror UI L onto UI R. **Do not** restyle texture; keep volume, length, and color.',
...
function salonPartSideViewFromFrontDonorBlock(
  ...
  const drapeHint =
    targetPart === 'RIGHT'
      ? ' Keep **UI R comb-over** on this **' +
        angleLabel +
        '**: part **image LEFT**, **image LEFT** shoulder mass from **behind** — **NOT** UI L top-sweep. **FORBIDDEN:** part **image RIGHT**.'
      : ' Keep **UI L**: part **image RIGHT**, forward sweep to **image LEFT** shoulder. **FORBIDDEN:** part **image LEFT**.';
  return (
    '**PART on this ' +
    angleLabel +
    ':** Show **' +
    partLabel +
    '** — same part + drape family as **IMAGE 2** (front donor) on this **turned** head.' +
    drapeHint +
    ' **FORBIDDEN:** center/MIDDLE part. **FORBIDDEN:** rotating head to **front** to show the part.'
  );
}
...
function curtainBangsAddonForSalonPart(partSelection: NoirLayersPartSelection): string {
  ...
  return (
    '**BANGS + salon:** **UI R** — part **image LEFT** forehead; **comb-over** lengths from **behind** on **image LEFT** shoulder — **NOT** UI L top-forward sweep. **FORBIDDEN:** part **image RIGHT** (**UI L**).'
  );
}
...
function buildLayersStylePromptShared(
  ...
  const partWordLayers =
    partSelection === 'MIDDLE'
      ...
      : partSelection === 'LEFT'
        ...
        : '**RIGHT part (UI “R”):** Part **image LEFT**. **Comb-over:** part + **image LEFT** shoulder **same side** — bulk from **behind**, **NOT** UI L top-forward sweep. **FORBIDDEN:** part **image RIGHT** (UI **L**).';
  ...
  const partWordCrimps =
    partSelection === 'MIDDLE'
      ...
      : partSelection === 'LEFT'
        ...
        : '**RIGHT part (UI “R”):** Part **image LEFT**. **Comb-over crimps** on **image LEFT** shoulder from **behind** — **NOT** UI L top-sweep. **FORBIDDEN:** part **image RIGHT** (UI L).';
    ...
    angle === 'left'
      ? partSelection === 'MIDDLE'
        ...
        : partSelection === 'LEFT'
          ...
          : salon === 'crimps'
            ? '**LEFT 3/4:** **UI R** — part **image LEFT**; **comb-over** on **image LEFT** shoulder from **behind**; **forbidden** part **image RIGHT**; **forbidden** UI L top-sweep.'
            : '**LEFT 3/4:** **UI R** — part **image LEFT**; **comb-over** waves on **image LEFT** shoulder; **forbidden** part **image RIGHT**; **forbidden** UI L cascade.'
      : angle === 'right'
        ? partSelection === 'MIDDLE'
          ? salon === 'crimps'
          ...
          : partSelection === 'LEFT'
            ? salon === 'crimps'
              ...
              : '**RIGHT 3/4:** **UI L** — part **image RIGHT**; bulk **viewer’s LEFT** shoulder only; **forbidden** part **image LEFT**.'
            : salon === 'crimps'
              ? '**RIGHT 3/4:** **UI R** — part **image LEFT**; **comb-over** on **image LEFT** shoulder; **forbidden** part **image RIGHT**; **forbidden** UI L drape.'
              : '**RIGHT 3/4:** **UI R** — part **image LEFT**; **comb-over** on **image LEFT** shoulder; **forbidden** part **image RIGHT**; **forbidden** UI L cascade.'
        : (() => {
            const oneShoulderFrontLayers =
              partSelection === 'MIDDLE'
                ...
                : partSelection === 'LEFT'
                  ...
                  : '**FRONT:** **UI R** — part **image LEFT**; **comb-over** on **image LEFT** shoulder from **behind** — **NOT** UI L top-sweep. **FORBIDDEN:** part **image RIGHT**.';
            const oneShoulderFrontCrimps =
              partSelection === 'MIDDLE'
                ? '**FRONT:** **MIDDLE** — crimps; **heaviest viewer’s LEFT** shoulder only.'
                : partSelection === 'LEFT'
                  ? '**FRONT:** **UI L** — part **image RIGHT**; forward crimps **viewer’s LEFT** shoulder. **FORBIDDEN:** part **image LEFT**.'
                  : '**FRONT:** **UI R** — part **image LEFT**; **comb-over crimps** from **behind** on **image LEFT** shoulder. **FORBIDDEN:** part **image RIGHT**.';
  ...
  return [
    ...
    salonPartDirectionSemanticsBlock(),
    bawSalonDrapeBlockForPart(partSelection),
    ...(salon === 'layers' ? [bawLayersUniformWaveTextureBlock()] : []),
...
function buildBawSalonFrontAnchorSideSupplement(
  ...
  const hairIdentityLine =
    partSelection === 'MIDDLE'
      ? '**same** wave/crimp **pattern and scale**, **same** part line, **same** layering, **same** drape'
      : '**same** wave/crimp **pattern and scale**, length, and volume — **part groove + drape from PART PLACEMENT rules below (prompt wins over IMAGE 1 if conflict)**';
  return [
    ...
    '**SIDE VIEW HAIR RULE (IMAGE 1 front lock):** **IMAGE 1** is the **already-styled FRONT (M)** for this **' +
      ...
      '** camera — ' +
      hairIdentityLine +
      '. **FORBIDDEN:** inventing a **new similar** hairstyle; **FORBIDDEN:** a different curl/wave family vs IMAGE 1; **FORBIDDEN:** “same vibe” or “matching style” — must be **the same hair** rotated to the side camera.',
    bawSalonModeLockBlock(salon),
    bawSalonFinishLookBlock(salon),
    ...(layersWave ? [layersWave] : []),
    ...(partSelection !== 'MIDDLE' ? [salonPartMustOverrideInputReferenceBlock(partSelection)] : []),
...
export function buildBawSalonStylingWithFrontAnchorPrompt(
  ...
  if (partSelection === 'RIGHT' && options?.hasRightPartPlacementRef) {
    return buildBawSalonRightPartSideViewWithPlacementRefPrompt(angle, salon, catalog, options);
  }
