...
import { aioEnv, getAioSupabaseConnectionLabel, isBackendConfigured } from '../../config/env';
...
export function getAioSupabase(): SupabaseClient | null {
  ...
  if (!client) {
    if (import.meta.env.DEV) {
      const label = getAioSupabaseConnectionLabel();
      if (label) console.info('[aio-supabase] client init', { project: label });
    }
