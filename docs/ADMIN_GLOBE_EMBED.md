...
**Look (embed):** Transparent page/canvas (iframe on marble). **Inner volume:** **`globeImageUrl`** set to a **PNG data URL** of the same **radial light→dark gray** canvas as the SVG — **three-globe**’s normal texture path (avoids the **no-image → black sphere** branch and avoids fragile **`globeMaterial`** swaps). **`waitForGlobeReady: true`** so the first frame is not black while the data URL loads. **`showGlobe(true)`**. **Land** = dark **neutral gray** dots on **GeoJSON**. **Hex bins** = hotspot pillars (**slate**). **Atmosphere** = soft **slate** halo. **Red** visitors / **green** orders.
