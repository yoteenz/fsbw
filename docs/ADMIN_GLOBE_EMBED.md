...
**Order cluster landmarks (zoomed in):** **`htmlElementsData`** shows one **glass / holographic mesh chip** per order cluster (mint→sky translucent gradient, fine grid overlay, soft cyan glow on the emoji). Tap opens **`MSG_CLUSTER`** + recenters. **Important:** `three-render-objects` sets the **CSS2D** overlay div to **`pointer-events: none`** by default, so landmark taps would pass through to the canvas; the embed sets **`pointer-events: auto`** on that overlay after init / resize / label updates so chips receive **`pointerup`**. **Fallback:** tapping the **green order dot** still calls **`activateOrderCluster`** via **`onPointClick`**.
