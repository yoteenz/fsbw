...
**Look (embed):** Transparent page/canvas (iframe on marble). **Ocean base:** **`globeImageUrl`** = very light PNG + **`onGlobeReady`** sets **`globeMaterial()`** **`transparent`**, **`opacity ~0.28`**, **`depthWrite: false`** so marble shows through. **Continents:** **~38k** land samples with **weight `800`** → **H3 `hexBinMerge(true)`** = one **merged honeycomb mesh** (flat prism tops, mint→sky by bin center lat). **Hex bins** do **not** fake order/view volume (no hot-bin jitter for counts). **Order / view volume:** **`customLayerData`** — each count is an **H3 hex prism** built from **`h3-js`** `cellToBoundary` at resolution **3** (same cell size as **`hexBinResolution(3.55)`**, which floors to **3**). Prisms sit **inside** the land honeycomb footprint (not **`CylinderGeometry`** points). **Order** layers use **`ADMIN_GLOBE_ORDER_PILLAR_RGBA`** (dark translucent blue); **view** layers use translucent brand red. **`onCustomLayerClick`** opens the order cluster panel (same as tapping the old green dot). Standalone visitors still use **`pointsData`** red dots. **`reorderGlobeLabelsAboveHex`** inserts the **custom** layer root **after** merged **hexBinPoints** so prisms render on top of land.
