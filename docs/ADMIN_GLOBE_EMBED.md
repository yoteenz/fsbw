...
**Look (embed):** Transparent page/canvas (iframe on marble). **Ocean base:** **`globeImageUrl`** = very light PNG + **`onGlobeReady`** sets **`globeMaterial()`** **`transparent`**, **`opacity ~0.28`**, **`depthWrite: false`** so marble shows through. **Continents:** **~38k** land samples with **weight `800`** → **H3 `hexBinMerge(true)`** = one **merged honeycomb mesh** (flat prism tops, mint→sky by bin center lat). **Hex bins** do **not** fake order/view volume (no hot-bin jitter for counts). **Order / view volume:** **`customLayerData`** — each count is an **H3 prism** from **`h3-js`** `cellToBoundary` at resolution **3** (same as **`hexBinResolution(3.55)`** floored). Footprints use the same **`hexMargin`** vertex inset as land **`hexBinMerge`** hexes, and cluster anchors are **snapped** to **`cellToLatLng(latLngToCell(...))`** so stacks align with one honeycomb cell. **Order** layers use **`ADMIN_GLOBE_ORDER_PILLAR_RGBA`** (dark translucent blue); **view** layers use translucent brand red. **`onCustomLayerClick`** opens the order cluster panel. Standalone visitors use **`pointsData`** red dots. **`reorderGlobeLabelsAboveHex`** places the **custom** layer after merged **hexBinPoints**.
