...
**Order cluster landmarks (zoomed in):** **`htmlElementsData`** shows one **glass / holographic mesh chip** per order cluster (mint→sky translucent gradient, fine grid overlay, soft cyan glow on the emoji). Tap opens **`MSG_CLUSTER`** + recenters (same as before). Plain **`onPointClick`** on the green dot is ignored for clusters so the chip receives the click.
