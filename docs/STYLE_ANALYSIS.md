...
**Default render (composite + Fal hair):** `HAIRSTYLE_ANALYSIS_RENDER_MODE=composite` (default) runs **GPT Image 2** hair-only edits on the client selfie and each match thumbnail (`api/_lib/hairstyleAnalysisHairGenerate.ts`), then composites the immutable Supabase template PNG (`live-preview/Analysis/IMG_2438|2447|2450|2451.png`) with **sharp** + bundled fonts — styled photos, Futura PT Medium spec/match text, gray `#808080` match-score %, Covered By Your Grace overall %, and star PNGs. Panel acrylic frost, brand-red glow, and marble are never AI-repainted. Output **4:5** at **2048×2560**. API: `POST /api/hairstyle-analysis-generate`. Optional `HAIRSTYLE_ANALYSIS_HAIR_CONCURRENCY` (default **3**) limits parallel thumb Fal calls.
