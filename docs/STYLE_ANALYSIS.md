...
**Post-process overlay (server):** Fal fills TOP MATCH spec values, match rows, and match-score % (medium gray `#808080` in template slot). Only **overall score %** and **match-rating stars** are left blank for `api/_lib/hairstyleAnalysisFalComposite.ts` (sharp). Part values are **MIDDLE / LEFT / RIGHT** only; style values use Build-a-Wig ids (**LAYERS**, **FLAT IRON**, **CRIMPS**, etc.). **COLOR** value fields show the catalog color name only (no hex or parentheses). Additional matches use varied styling. **Why it works / every detail matters:** Fixed template structure — exactly N rose-icon rows, each one **verbatim sentence** tying client features to selected specs (no label:value rows, no empowerment fluff). **Salon styling:** Fal attaches BAW `after-color` styling reference WebPs (same paths as `live-wig-after-color-styling`) — copy hairstyle shape only, retint to catalog hex. Fal prompt preserves acrylic panel frost and red panel glow from the static template.
