...
**Default render (reference template + Fal hair):** `HAIRSTYLE_ANALYSIS_RENDER_MODE=composite` (default) uses the **Supabase reference PNG** per tier (`IMG_2438|2447|2450|2451` — marble, acrylic, red glow, footer, BUILD THIS LOOK baked in) as the chrome base, runs **GPT Image 2** hair-only edits (`hairstyleAnalysisHairGenerate.ts` — `quality: medium`, **3:4** `1536×2048` per edit, `auto` fallback), then composites styled photos + **value-only** overlays with **sharp** at **2048×2560 (4:5)**. Slot map: `api/_lib/hairstyleAnalysisCardBlueprint.ts` (synced to `src/data/hairstyleAnalysisCardBlueprint.json`). API: `POST /api/hairstyle-analysis-generate`. `HAIRSTYLE_ANALYSIS_HAIR_CONCURRENCY` (default **3**).
