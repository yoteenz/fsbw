...
1. **Hair-only step:** Selfie + inspo → client wearing exact inspo hairstyle (geometry from inspo, identity from selfie), aligned to manual/manifest specs but with the photo as the visual source of truth. Long hair uses a **soft asymmetric drape**: fuller front cascade on one side plus visible back/behind-shoulder hair on the other side (balanced ~60/40), never equal thick forward curtains and never all hair piled on one shoulder.
2. **Template pass:** `six_month` → free 1-pick card; `twelve_month` → premium 4-pick card with color-only MATCH 02–04 thumbs. Both consult templates use the consult inspo lock; BAW styling refs are secondary and must not replace the photographed length, bangs/fringe, curl/crimp pattern, part, soft asymmetric drape, or silhouette.
