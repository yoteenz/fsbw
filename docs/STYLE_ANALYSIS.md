...
**In-image generation:** Fal fills TOP MATCH specs, photos, and every-detail-matters rows. **Server post-process** (`hairstyleAnalysisFalComposite.ts`): **client preview bottom fade** (fixed symmetrical alpha mask — template marble/panel shows through; Fal must not paint clouds/mist), **overall score %** (Covered By Your Grace + Futura % at slot height × 0.78 − **4px**), **match-rating stars** (Noir `filled-star.png` / `star-symbol.png` at per-star positions pixel-probed on IMG_2549 — filled stars only on premium; all five on free), and **MATCH 02–04 row values** (Futura at slot height × 0.88 − 2px). Fal leaves those slots blank. **Fal image refs (default):** template + client selfie + **BAW styling refs** (LAYERS / CRIMPS / FLAT IRON shape). Unit mannequin fronts are **opt-in** (`HAIRSTYLE_ANALYSIS_FAL_MANNEQUIN_REFS=true`) — default off to avoid mannequin neck/shoulder bleed; one-shoulder drape is prompt-led. Set `HAIRSTYLE_ANALYSIS_FAL_MINIMAL_REFS=true` to skip styling refs (template + client only). Part values are **MIDDLE / LEFT / RIGHT** only; style values use Build-a-Wig ids (**LAYERS**, **FLAT IRON**, **CRIMPS**, etc.). **COLOR** value fields show the catalog color name only (no hex or parentheses). Hair edits use **strand-level recolor** instructions (no flat overlay). **Every detail matters:** Fixed rose-icon rows — print **EVERY DETAIL MATTERS LINE** sentences verbatim (one facial feature + one spec per row; not motivational “why it works” copy). Fal prompt prioritizes preserving acrylic frost, **brand-red panel glow**, and marble texture from the static template.
