...
**Allowance:** **3 / 6 / 12 month** premium subscribers get **1 free hairstyle analysis per UTC calendar month** (card tier matches subscription). API: `GET /api/hairstyle-analysis-usage`, enforced on `POST /api/hairstyle-analysis-generate` (refunds slot if generation fails). Admins bypass the cap for testing.
...
**Default render (server composite):** `HAIRSTYLE_ANALYSIS_RENDER_MODE=composite` (default) loads the immutable Supabase template PNG (`live-preview/Analysis/IMG_2438|2447|2450|2451.png`) and composites all dynamic content with **sharp** + bundled fonts — client preview photo, match thumbnails (look `imageUrl` or unit mannequin fallback), Futura PT Medium spec/match text, gray `#808080` match-score %, Covered By Your Grace overall %, and star PNGs. Panel acrylic frost, brand-red glow, and marble are never AI-repainted. Output **4:5** at **2048×2560**. API: `POST /api/hairstyle-analysis-generate`. Code: `api/_lib/hairstyleAnalysisCompositeCard.ts`, `hairstyleAnalysisLayoutFields.ts`, `hairstyleAnalysisOverlayValues.ts`, `hairstyleAnalysisFonts.ts`.

**Legacy Fal path:** Set `HAIRSTYLE_ANALYSIS_RENDER_MODE=fal` to use GPT Image 2 (`openai/gpt-image-2/edit`) full-template edit + partial post-process in `api/_lib/hairstyleAnalysisFalComposite.ts` (overall % + stars only). **Fal image refs:** default **minimal** — template + client photo only (`HAIRSTYLE_ANALYSIS_FAL_MINIMAL_REFS=true`; set `false` to re-attach mannequin/styling refs).
