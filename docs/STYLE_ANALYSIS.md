...
**Default render (built template + Fal hair):** `HAIRSTYLE_ANALYSIS_RENDER_MODE=composite` (default) **renders the card chrome in code** (`api/_lib/hairstyleAnalysisCardBlueprint.ts` + `hairstyleAnalysisBuiltTemplate.ts` — marble, frosted panels, red glow, static labels, roses) at **2048×2560**, runs **GPT Image 2** hair-only edits (`hairstyleAnalysisHairGenerate.ts`), then composites styled photos + dynamic values with **sharp** + bundled fonts. Labels and value slots share one blueprint so alignment is exact — **not** the legacy Supabase `IMG_2438|2447|2450|2451.png` assets. Dev overlay slots: `src/data/hairstyleAnalysisCardBlueprint.json` (synced from blueprint). API: `POST /api/hairstyle-analysis-generate`. Optional `HAIRSTYLE_ANALYSIS_HAIR_CONCURRENCY` (default **3**).
