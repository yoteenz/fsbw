...
**In-image generation:** Fal fills the entire template — TOP MATCH specs, match rows, gray match-score %, **overall score %** (handwritten script style similar to Covered By Your Grace in brand red `#EB1C24`), and **match-rating stars** (fill pre-rendered star outlines on the premium template with `#EB1C24`; on the free template draw matching star outlines then fill earned stars). No server post-process overlay. **Fal image refs:** **unit mannequin fronts always attached** (texture + one-shoulder drape silhouette). Optional BAW styling refs when `HAIRSTYLE_ANALYSIS_FAL_MINIMAL_REFS` is unset/false; set `HAIRSTYLE_ANALYSIS_FAL_MINIMAL_REFS=true` to skip styling refs only. Part values are **MIDDLE / LEFT / RIGHT** only; style values use Build-a-Wig ids (**LAYERS**, **FLAT IRON**, **CRIMPS**, etc.). **COLOR** value fields show the catalog color name only (no hex or parentheses). Hair edits use **strand-level recolor** instructions (no flat overlay). **Every detail matters:** Fixed rose-icon rows — print **EVERY DETAIL MATTERS LINE** sentences verbatim (one facial feature + one spec per row; not motivational “why it works” copy). Fal prompt prioritizes preserving acrylic frost, **brand-red panel glow**, and marble texture from the static template.
