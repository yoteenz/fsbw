...
On **`/build-a-wig/noir/edit/styling`** or **`.../customize/styling`**, when admin + Supabase session and **exactly one** salon style among **LAYERS**, **CRIMPS**, **FLAT IRON** and part **MIDDLE**, **LEFT**, or **RIGHT** are selected, the app calls **`POST /api/live-wig-after-color-styling`** **three times in sequence** with body **`angle`**: `"front"` then `"left"` then `"right"` (one fal job per serverless invocation). Body includes **`partSelection`**. **MIDDLE part:** L/R camera angles anchor to that part’s **FRONT (M)** styled output via **`buildBawSalonStylingWithFrontAnchorPrompt`** + **`[ front styled, gray-brick side ]`**. **UI L / UI R part:** **FRONT (M)** re-parts from **MIDDLE-part FRONT**. **L/R cameras:** **same chain as MIDDLE part** — **`buildBawSalonStylingWithFrontAnchorPrompt`** + **`[ side-part FRONT (M), gray-brick side ]`** (gray-brick **IMAGE 2** = mannequin body/camera lock; front **IMAGE 1** = hair identity only). **Prerequisites:** **MIDDLE** part **FRONT**; then side-part **FRONT (M)** before side-part **L/R**. A **single** no-`angle` request that runs three fal steps in one function often hits Vercel **`FUNCTION_INVOCATION_FAILED`** / duration limits. **`forceRegenerate`** on **regen style L/M/R** unchanged. Resolution: **`WIG_PREVIEW_FAL_STYLING_RESOLUTION`** or **`WIG_PREVIEW_FAL_RESOLUTION`**, default **2K** for this route.

**LAYERS mode:** **Single** `image_urls` = **color-tier WebP** from Storage (same paths as live color — hair already matches the swatch). Prompt: **`buildLayersStylePromptFromColorTierWebp`** — **`STYLE LOCK`** block: same canonical silhouette/texture for **every** catalog color (ignore input-shape drift between swatches); **uniform voluminous layered S-waves** (one wave scale, **not** ringlets/spiral curls/curl clusters); **explicitly keep** the tinted hair color (no black revert); **part** = **image LEFT** vs **image RIGHT** (thumbnail) for scalp placement (**UI L** → part on **image RIGHT**); plus **`DRAPE SIDE`**: main forward panel over **viewer’s LEFT** shoulder (left side of image); length **behind** falls **straight down the back** naturally — **forbidden** sweeping all back hair sideways to one side; **forbidden** thick drape on **viewer’s RIGHT** shoulder; **shoulder cap/curve** should still **read through** the forward drape (strands/gaps, not a solid opaque wall). **Prerequisite:** color L/F/R WebPs must exist (generate on NOIR → Color first). **No** `WIG_PREVIEW_NOIR_MANNEQUIN_*` requirement for LAYERS.

**CRIMPS mode:** Same **single** color-tier WebP input as LAYERS. Prompt: **`buildCrimpsStylePromptFromColorTierWebp`** — **horizontal accordion / waffle ridges**, **high-gloss**, **extra-long** hair. **FRONT (hero):** same **asymmetric** one-shoulder rules as LAYERS (no thick symmetric panels on **both** shoulders). Side parts: **sleek roots** + **one** heavy-side cascade. Keeps **catalog swatch** hair color. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/crimps-{middle|left|right}-part/{left|front|right}.webp`.

**FLAT IRON mode:** **Color-tier WebP** as **Image 1** (swatch). **MIDDLE part, no bangs:** **`image_urls`** = **[ color WebP, Fal gray-brick mannequin ]** — same refs as live color (`api/_lib/bawNoirFalMannequinUrls.ts`: `fal-gray-brick-{left|front|right}.png`). **LEFT/RIGHT** or **+ bangs:** single color WebP only (unchanged). **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/flat-iron-{middle|left|right}-part/{left|front|right}.webp`.
...
**BANGS + LAYERS, BANGS + CRIMPS, or BANGS + FLAT IRON:** Body **`styling`** includes **`BANGS`** and **one** salon style (e.g. `BANGS,LAYERS`). Same salon prompts with **`includeBangs: true`** — **curtain bangs** aligned to **`partSelection`** (center split for MIDDLE; asymmetric for LEFT/RIGHT). **Output:** `.../after-color/layers-with-bangs-{middle|left|right}-part/`, `crimps-with-bangs-{middle|left|right}-part/`, or `flat-iron-with-bangs-{middle|left|right}-part/`. Separate cache from salon-only and from **bangs-only**.

**Bangs only (BANGS without LAYERS/CRIMPS/FLAT IRON):** Same API and body shape; **`styling`** must be **`BANGS`** only (no **`LAYERS`**, **`CRIMPS`**, or **`FLAT IRON`**). Fal uses **one** `image_url` (the **color-tier WebP** from Storage) and **`buildBangsOnlyStylePrompt`**. **Output:** `.../after-color/bangs-only/{angle}.webp`. Hub shows bangs preview after salon+part in priority order.

**Prerequisite:** Run **NOIR → Color** (admin live preview) first so the three color WebPs exist — required for **LAYERS**, **CRIMPS**, **FLAT IRON**, **combined BANGS+salon**, and **BANGS-only**. To fix a bad angle: **regen style L / M / R** (or delete that WebP in Storage and reload).
