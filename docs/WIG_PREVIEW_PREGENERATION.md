...
**Vercel env (required for live feature):** `FAL_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `WIG_PREVIEW_STORAGE_BUCKET`, plus optional public **`WIG_PREVIEW_NOIR_MANNEQUIN_*_URL`** overrides — see `.env.example`. When unset, the API uses **Supabase** `live-preview/Noir/image (26|27|28).png` (M/L/R). **Do not** set these to legacy `https://…/assets/natural *.png` on the app origin. Verify reachability: **`npm run wig-preview:verify-noir-mannequins`**. Bucket must allow **public read** (or extend the API to return signed URLs). Optional **`WIG_PREVIEW_FAL_RESOLUTION`**: **`4K`** (default), **`2K`**, or **`1K`** (use lower if Vercel Hobby timeouts or you want cheaper/faster runs). The API still accepts a request **without** `angle` (all three angles in one invocation) for Pro / long `maxDuration`.
...
**FLAT IRON mode:** **Color-tier WebP** as **Image 1** (swatch). **MIDDLE part, no bangs:** **`image_urls`** = **[ color WebP, Supabase NOIR mannequin for that angle ]** — same public URLs as live color (`api/_lib/bawNoirNaturalMannequinUrls.ts`: `image (27)` L, `image (26)` M, `image (28)` R); second ref locks **geometry / camera angle** to the BAW hub base angles. Prompt: `buildFlatIronStylePromptFromColorTierWebp` with **`baseNoirGeometrySecondRef`**. **LEFT/RIGHT** or **+ bangs:** single color WebP only (unchanged). Prompt: **bone-straight / flat-ironed** + **part line**; **no** waves, curls, or crimps. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/flat-iron-{middle|left|right}-part/{left|front|right}.webp`.
