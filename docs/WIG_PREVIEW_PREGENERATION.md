...
**LAYERS mode:** **Single** `image_urls` = **color-tier WebP** from Storage (same paths as live color — hair already matches the swatch). Prompt: **`buildLayersStylePromptFromColorTierWebp`** — **voluminous layered S-waves** (large soft waves, brushed-out barrel curls, **blended** flow — **not** tight separated ringlets); **explicitly keep** the tinted hair color (no black revert); **part** = **frame LEFT** / **frame RIGHT** (photo edges) for **LEFT** / **RIGHT** `partSelection` so the model does **not** mirror the part; **one-sided** drape rules. **Prerequisite:** color L/F/R WebPs must exist (generate on NOIR → Color first). **No** `WIG_PREVIEW_NOIR_MANNEQUIN_*` requirement for LAYERS.
