...
**CRIMPS mode:** Same **single** color-tier WebP input as LAYERS. Prompt: **`buildCrimpsStylePromptFromColorTierWebp`** — aligns with **crimps ref images**: **horizontal accordion / waffle ridges**, **high-gloss**, **extra-long** hair; **MIDDLE** = symmetric drape over **both** shoulders; **LEFT/RIGHT** = **sleek roots** at part + **one-sided shoulder drape** on the heavy side. Keeps **catalog swatch** hair color (not ref black/burgundy). **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/crimps-{middle|left|right}-part/{left|front|right}.webp`.
