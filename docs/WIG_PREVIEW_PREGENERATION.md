...
On **`/build-a-wig/noir/edit/styling`** or **`.../customize/styling`**, when admin + Supabase session and **exactly one** salon style among **LAYERS**, **CRIMPS**, **FLAT IRON** and part **MIDDLE**, **LEFT**, or **RIGHT** are selected, the app calls **`POST /api/live-wig-after-color-styling`** **three times in sequence** with body **`angle`**: `"front"` then `"left"` then `"right"` (one fal job per serverless invocation). Body includes **`partSelection`**. **MIDDLE part:** L/R camera angles anchor to that part’s **FRONT (M)** styled output via **`buildBawSalonStylingWithFrontAnchorPrompt`**. **UI L / UI R part:** **FRONT** re-parts from **MIDDLE-part FRONT**; **L/R cameras** re-part from **MIDDLE-part same-angle** (**middle LEFT 3/4** / **middle RIGHT 3/4**) via **`buildBawSalonSidePartFromMiddleSameAnglePrompt`** — **not** side-part FRONT (that re-rolled wrong handedness on **R**). **Prerequisites:** **MIDDLE** part **all three angles** first; then **LEFT/RIGHT** part triple. A **single** no-`angle` request that runs three fal steps in one function often hits Vercel **`FUNCTION_INVOCATION_FAILED`** / duration limits. **`forceRegenerate`** on **regen style L/M/R** unchanged. Resolution: **`WIG_PREVIEW_FAL_STYLING_RESOLUTION`** or **`WIG_PREVIEW_FAL_RESOLUTION`**, default **2K** for this route.
