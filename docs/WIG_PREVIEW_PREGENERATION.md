...
**UI RIGHT + salon:** Fal uses **`openai/gpt-image-2/edit`** (slower than nano-banana). The route uses **`maxDuration: 300`** in **`api/live-wig-after-color-styling.ts`** and **`vercel.json`** so a single angle is less likely to hit **`FUNCTION_INVOCATION_TIMEOUT`**. Requires a Vercel plan that allows **300s** (Pro); Hobby caps lower. GPT Image 2 **`quality`** is **`medium`** for this path to reduce queue time.
...
**FLAT IRON mode:** **Color-tier WebP** as **Image 1** (swatch). **MIDDLE part, no bangs:** **`image_urls`** = **[ color WebP, public `/assets/natural {left|front|right}.png` ]** — second ref locks **geometry / camera angle** to the same **base NOIR** static angles as the BAW hub; prompt includes two-image instructions (`buildFlatIronStylePromptFromColorTierWebp` with **`baseNoirGeometrySecondRef`**). **LEFT/RIGHT** or **+ bangs:** single color WebP only (unchanged). **Env:** **`WIG_PREVIEW_PUBLIC_APP_ORIGIN`** or **`SITE_URL`** (else **`https://fsbw.vercel.app`**) so fal can fetch the static asset. Prompt: **bone-straight / flat-ironed** + **part line**; **no** waves, curls, or crimps. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/flat-iron-{middle|left|right}-part/{left|front|right}.webp`.
