...
On **`/build-a-wig/noir/edit/styling`** or **`.../customize/styling`**, when admin + Supabase session and **LAYERS** + part **MIDDLE** are selected, the app sends **one** **`POST /api/live-wig-after-color-styling`** with **no** `angle` body field; the server runs **front → left → right** in one invocation (avoids three cold starts / **`FUNCTION_INVOCATION_FAILED`**). Optional body `angle` + **`forceRegenerate`** still powers **regen style L/M/R**. Resolution: **`WIG_PREVIEW_FAL_STYLING_RESOLUTION`** or **`WIG_PREVIEW_FAL_RESOLUTION`**, default **2K** for this route (two-image fal is heavier than live color). Live **color** still uses **three parallel** one-angle requests. Fal uses **`BAW_MIDDLE_PART_LAYERS_STYLE_PROMPT_TWO_IMAGES`** and **`image_urls`**: **[1]** color WebP, **[2]** style inspo from **`WIG_PREVIEW_NOIR_MIDDLE_LAYERS_STYLE_FRONT_URL`**, **`_LEFT_`**, **`_RIGHT_`**.
