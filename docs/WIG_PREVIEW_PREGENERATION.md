...
**CRIMPS mode:** Same **single** color-tier WebP input as LAYERS. Prompt: **`buildCrimpsStylePromptFromColorTierWebp`** — **salon deep-pressed crimps** (continuous zig-zag ridges, even spacing, glossy — **not** loose waves or spiral curls). **MIDDLE** front: balanced crimp panels on both sides of center part; **LEFT/RIGHT**: one-sided drape per color-page rules. Keeps swatch hair color. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/crimps-{middle|left|right}-part/{left|front|right}.webp`.
