...
**CRIMPS mode:** Same **single** color-tier WebP input as LAYERS. Prompt: **`buildCrimpsStylePromptFromColorTierWebp`** — **horizontal accordion / waffle ridges**, **high-gloss**, **extra-long** hair. **FRONT (hero):** **one-shoulder drape** for **MIDDLE**, **LEFT**, and **RIGHT** (not symmetric thick panels on both shoulders). Side parts: **sleek roots** + heavy-side drape. Keeps **catalog swatch** hair color. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/crimps-{middle|left|right}-part/{left|front|right}.webp`.
