...
**FLAT IRON mode:** Same **single** color-tier WebP input. Prompt: **`buildFlatIronStylePromptFromColorTierWebp`** — **bone-straight / flat-ironed** finish + **part line** (MIDDLE / LEFT / RIGHT); **no** waves, curls, or crimps — treat as the **same base color image** with **only** part + straightening edits. **Output paths:** `wig-preview-live/{v}/NOIR/{colorTierHash}/after-color/flat-iron-{middle|left|right}-part/{left|front|right}.webp`.
