...
export function resolveDockContext(pathname: string): DockContextProfile {
  ...
  if (pathname.includes('/studio-foundation-models')) {
    const workspaceId = getRuntimeActiveWorkspaceId();
    return {
      contextId: 'studio-foundation-models',
  ...
  if (pathname.includes('/permission-engine')) {
    const workspaceId = getRuntimeActiveWorkspaceId();
    return {
      contextId: 'permission-engine',
      label: 'PERMISSION ENGINE™ · CAPABILITIES',
      portfolioMode: false,
      suggestedCommands: listPermissionEngineDockSuggestions(workspaceId),
      commandTypes: ['executive-requests', 'organization-settings', 'knowledge-search'],
    };
  }

  if (pathname.includes('/policy-engine')) {
    const workspaceId = getRuntimeActiveWorkspaceId();
    return {
      contextId: 'policy-engine',
      label: 'POLICY ENGINE™ · ORGANIZATIONAL LAW',
      portfolioMode: false,
      suggestedCommands: listPolicyEngineDockSuggestions(workspaceId),
      commandTypes: ['executive-requests', 'organization-settings', 'knowledge-search'],
    };
  }
  ...
  if (pathname.includes('/component-registry')) {
    const workspaceId = getRuntimeActiveWorkspaceId();
    return {
      contextId: 'component-registry',
      label: 'COMPONENT REGISTRY™ · REUSE FIRST',
      portfolioMode: false,
      suggestedCommands: listComponentRegistryDockSuggestions(workspaceId),
      commandTypes: ['knowledge-search', 'creative-requests', 'organization-settings'],
    };
  }
