import { DIFFICULTY_WEIGHT, REFRESHER_MODES, STATUS_THRESHOLDS } from '../constants';
import type {
...
import { resolveReviewTriggers } from './triggers';
...
export function mode(id: RefresherModeId): RefresherMode {
  return REFRESHER_MODES.find((entry) => entry.id === id)!;
}
...
function baseModesForEvaluation(
  ...
  if (profile.certificationStatus === 'renewal-due' || profile.certificationRelevance >= 80) {
    modes.push(mode('certification-renewal'));
  }
  if (profile.upcomingSimulationIds.length > 0) modes.push(mode('simulation-replay'));
  if (profile.upcomingProjectIds.length > 0) modes.push(mode('interactive-scenario'));
  if (profile.mistakesMade >= 5 || risk >= STATUS_THRESHOLDS.critical) {
    modes.push(mode('mentor-walkthrough'), mode('quick-assessment'));
  } else if (risk >= STATUS_THRESHOLDS.needsRefresh) {
    modes.push(mode('tldr-review'));
  } else {
    modes.push(mode('memory-spark'));
  }
...
function buildReasons(
  ...
  else if (daysSinceRealUsage >= 180) reasons.push(`${daysSinceRealUsage} days since last practice.`);
  else if (daysSinceRealUsage >= 60) reasons.push(`${daysSinceRealUsage} days since last practice.`);

  if (profile.recallScore < 65) reasons.push(`Recall score is ${profile.recallScore}%.`);
  if (profile.confidenceScore < 70) reasons.push(`Confidence score is ${profile.confidenceScore}%.`);
  if (profile.mistakesMade >= 3) reasons.push(`${profile.mistakesMade} recent mistakes recorded.`);
  if (hasIndustryUpdate) reasons.push('Profession Brain™ has a relevant industry update.');
  if (profile.certificationStatus === 'renewal-due') reasons.push('Certification renewal is approaching.');
  if (profile.upcomingSimulationIds.length > 0) {
    reasons.push('Upcoming simulation can refresh this memory naturally.');
  }
  if (profile.upcomingProjectIds.length > 0) {
    reasons.push('Upcoming real-world project can apply this memory.');
  }
...
export function calculateRetentionRisk(
  ...
  const daysSinceRealUsage = daysBetween(profile.lastPracticed ?? profile.lastRealUsageAt, now);
  ...
  const weakRecallRisk = (100 - profile.recallScore) * 0.34;
  const lowConfidenceRisk = (100 - profile.confidenceScore) * 0.24;
  const usageProtection = Math.min(24, profile.applicationsCompleted * 1.15);
  const certificationRisk = profile.certificationRelevance * 0.08;
  const mistakeRisk = Math.min(18, profile.mistakesMade * 2.5);
...
export function evaluateRetentionProfile(
  ...
  const masteryScore = clamp(
    profile.confidenceScore * 0.36 +
      profile.recallScore * 0.36 +
      Math.min(100, profile.applicationsCompleted * 5) * 0.18 +
      (100 - risk) * 0.1
  );
  ...
  const triggers = resolveReviewTriggers(profile, { now, hasIndustryUpdate, daysSinceRealUsage });
...
export function buildOrbMentorLine(
  ...
  if (context.hasIndustryUpdate) {
    return `A new industry technique is now available for ${profile.conceptTitle.toLowerCase()}.`;
  }

  if (profile.upcomingProjectIds.length > 0 && context.status !== 'fresh') {
    return `Before today's first appointment, ${profile.conceptTitle.toLowerCase()} could use a quick refresh.`;
  }
  ...
  if (context.daysSinceRealUsage !== null && context.daysSinceRealUsage >= 60) {
    return `It's been a while since your last ${profile.conceptTitle.toLowerCase()}.`;
  }

  if (profile.applicationsCompleted >= 10 && context.status !== 'fresh') {
    return `You've mastered ${profile.conceptTitle.toLowerCase()} before—want a quick refresh?`;
  }
  ...
  return `A quick memory spark would keep ${profile.conceptTitle.toLowerCase()} alive.`;
}
