...
function MembershipPage() {
  ...
  useEffect(() => {
    ...
    const handleCustomCurrencyChange = (event: CustomEvent) => {
      ...
      if (newCurrency && currencyRates[newCurrency as keyof typeof currencyRates]) {
        ...
        try { const key = getPerUserKey(PER_USER_KEYS.selectedCurrency, getCurrentUserEmailFromStorage()); localStorage.setItem(key, newCurrency); } catch (_) {}
      }
    };
  ...
  const handleClosePremiumView = () => {
    ...
    setSelectedTier(null);
    setShowPremiumUpgradeAllBenefits(false);
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div style={{ textAlign: 'center', marginBottom: '12px', marginTop: '10px' }}>
                          <img src="/assets/premium-rewards.png" alt="" style={{ width: '182.16px', height: '182.16px', marginTop: '16px', marginBottom: '2px', display: 'block', marginLeft: 'auto', marginRight: 'auto', objectFit: 'contain' }} />
                          <p
                            style={{
                              fontFamily: '"Futura PT Medium"',
                              color: BRAND_GRAY,
                                  ...
                                  return (
                                    <div style={{ marginTop: '40px', paddingBottom: '20px' }}>
                                      ...
                                      <p style={{ fontFamily: '"Futura PT Book", futuristic-pt, Futura, Inter, sans-serif', fontWeight: '400', color: '#000000', fontSize: '10px', margin: '0', textTransform: 'uppercase' }}>
                                        {nextReward
                                          ? <>{((nextReward.points - totalPoints).toLocaleString())} MORE POINTS TO EARN {nextReward.type === 'digital_cash' ? <span style={{ color: nextLabelColor, fontFamily: '"Futura PT Medium", futuristic-pt, Futura, Inter, sans-serif', fontWeight: '500' }}>{nextReward.label}</span> : <>A <span style={{ color: nextLabelColor, fontFamily: '"Futura PT Medium", futuristic-pt, Futura, Inter, sans-serif', fontWeight: '500' }}>{nextReward.label}</span></>}!</>
                                          : 'MAX REWARD REACHED'}
                                      </p>
                                    </div>
                                  );
