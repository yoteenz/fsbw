...
const BLANCO_2D_ANGLE_DOWNLOAD_ROWS: { href: string; label: string; download: string }[] = [
  { href: '/assets/2D%20BLANCO%20LEFT.png', label: 'LEFT (L)', download: '2d-blanco-left.png' },
  { href: '/assets/2D%20BLANCO%20FRONT.png', label: 'FRONT (M)', download: '2d-blanco-front.png' },
  { href: '/assets/2D%20BLANCO%20RIGHT.png', label: 'RIGHT (R)', download: '2d-blanco-right.png' },
];
const BLANCO_2D_VIEWER_DOWNLOADS: ImageViewerDownloadLink[] = BLANCO_2D_ANGLE_DOWNLOAD_ROWS.map((r) => ({
  href: r.href,
  label: r.label,
  download: r.download,
}));
...
function BlancoSelection() {
  ...
  // Check if BLANCO is in wishlist on mount and when wishlist changes
  useEffect(() => {
    const checkWishlist = () => {
      try {
  ...
  const handleToggleWishlist = () => {
    try {
      const wishlistItems = JSON.parse(localStorage.getItem('wishlistItems') || '[]');
      const totalPrice = parseInt(localStorage.getItem('blancoTotalPrice') || '820');
  ...
  const [activeTab, setActiveTab] = usePersistentQueryState<'DETAILS' | 'SHIPPING' | 'POLICY' | 'CARE/STORAGE' | 'REVIEWS'>({
    queryKey: 'tab',
    storageKey: 'productBlancoActiveTab',
  ...
  const handleAddToBag = async () => {
    ...
    if (isWigUnitSoldOut('BLANCO') || isUnitSoldOut('BLANCO')) return;
          ...
          >
            ...
            <div className="product-wig-preview" style={{ width: '100%', display: 'flex', alignItems: 'center', flexDirection: 'column', marginBottom: '24px', transform: 'translateY(20px)', overflow: 'visible', minWidth: '100%', maxWidth: 'none' }}>
              ...
              <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'stretch' }}>
                {/* ADD TO WISHLIST & 2D/3D - same width as image block; text near edges */}
                  ...
                  <div style={{ position: 'relative', overflow: 'visible', flexShrink: '0' }}>
                    <div
                      style={{
                        ...
                        backgroundImage: `url('${is3DView ? current3DImages.hero : '/assets/leaf-brick-resize.png'}')`,
                        backgroundRepeat: 'repeat',
                        overflow: 'visible',
                        cursor: 'pointer'
                      }}
                      onClick={() => {
                        ...
                        setViewerModalDownloads(!is3DView && showAdminFounder2dDownloads ? BLANCO_2D_VIEWER_DOWNLOADS : null);
...
width: 'clamp(230px, 57.5vw, 368px)',
                  ...
                  <div className="flex flex-col" style={{ height: 'clamp(290px, 72.5vw, 464px)', justifyContent: 'space-between', gap: 'clamp(6px, 1.2vw, 12px)' }}>
                    <div className="flex-shrink-0 relative">
                      <div
                        ...
                        style={{ width: 'clamp(100px, 26vw, 175px)', height: 'clamp(140px, 36vw, 245px)', backgroundImage: `url('${is3DView ? current3DImages.top : '/assets/leaf-brick-resize.png'}')`, backgroundSize: 'cover', backgroundPosition: is3DView ? 'center calc(50% + 5px)' : 'center', backgroundRepeat: 'no-repeat' }}
                    ...
                    <div className="flex-shrink-0 relative">
                      <div
                        className="relative bg-cover bg-center flex items-center justify-center cursor-pointer"
                        style={{ width: 'clamp(100px, 26vw, 175px)', height: 'clamp(140px, 36vw, 245px)', backgroundImage: `url('${is3DView ? current3DImages.bottom : '/assets/leaf-brick-resize.png'}')`, backgroundSize: 'cover', backgroundPosition: is3DView ? 'center calc(50% + 5px)' : 'center', backgroundRepeat: 'no-repeat' }}
              ...
              <div className="mb-6">
                ...
                <div className="flex justify-center gap-3" style={{ transform: 'translateY(-24px)' }}>
                  <button 
                    ...
                    className={`px-6 py-1 bg-white ${selectedCustomCap === 'XS' ? '' : 'hover:bg-gray-50'}`}
                    style={{ 
                  ...
                  <button 
                    ...
                    className={`px-6 py-1 bg-white ${selectedCustomCap === 'S' ? '' : 'hover:bg-gray-50'}`}
                    style={{ 
                  ...
                  <button 
                    ...
                    className={`px-6 py-1 bg-white ${selectedCustomCap === 'M' ? '' : 'hover:bg-gray-50'}`}
                    style={{ 
                  ...
                  <button 
                    ...
                    className={`px-6 py-1 bg-white ${selectedCustomCap === 'L' ? '' : 'hover:bg-gray-50'}`}
                    style={{ 
              ...
              {showAdminFounder2dDownloads ? (
                ...
                >
                  ...
                  <div className="flex flex-wrap justify-center gap-x-3 gap-y-1" style={{ maxWidth: '100%' }}>
                    {BLANCO_2D_ANGLE_DOWNLOAD_ROWS.map(({ href, label, download }) => (
                      <a
                        key={href}
                        href={href}
                        download={download}
          ...
          <UnitPdpCartActions
            ...
            onCustomize={() => {
                ...
                const capSizeToSave = selectedCustomCap || selectedFlexibleCap || 'M'; // Default to 'M' if nothing selected
                ...
                console.log('Blanco customize mode - Setting cap size:', capSizeToSave, 'price:', capSizePrice);
                
                // Set defaults for other selections so customize page loads with defaults + selected cap
                // BLANCO-specific defaults
                const defaults = {
                  length: '24"',
                  density: '250%', // BLANCO default
                  lace: '13X6',
                  texture: 'SILKY',
                  color: 'PLATINUM', // BLANCO default
                  hairline: 'NATURAL',
                  styling: 'NONE',
                  addOns: [],
                };
