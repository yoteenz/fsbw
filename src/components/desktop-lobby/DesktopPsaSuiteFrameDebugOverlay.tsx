...
/** QA square on PSA Suite hologram — drag + corner/edge resize with save. */
export function DesktopPsaSuiteFrameDebugOverlay({
  measureRef,
  mappedRect,
  screenOffsetX,
  screenOffsetY,
  ...
  const onOverlayPointerDown = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (target.dataset.desktopPsaSuiteEdge || target.dataset.desktopPsaSuiteCorner) return;
  ...
  return (
    <div
      data-desktop-psa-suite-frame-debug
      style={{
    ...
    >
      <span style={DEBUG_LABEL_STYLE}>psa suite holo</span>
      {canGesture
        ? EDGE_HANDLES.map(({ edge, cursor, style }) => (
            <div
              ...
              role="presentation"
              aria-hidden
              data-desktop-psa-suite-edge={edge}
      ...
      {canGesture
        ? CORNER_HANDLES.map(({ corner, cursor, style }) => (
            <div
              ...
              role="presentation"
              aria-hidden
              data-desktop-psa-suite-corner={corner}
