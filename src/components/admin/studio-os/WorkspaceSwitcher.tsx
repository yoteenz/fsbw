...
import { StudioCommandCenterCard } from './StudioCommandCenterCard';
...
export function WorkspaceSwitcher() {
  ...
  const switcherDropdown = portfolioMode && open ? (
    <div
  ...
  if (portfolioMode) {
    return (
      <div className="grid grid-cols-2 gap-2 mb-2">
        <StudioCommandCenterCard />
        <div className="relative min-w-0">
          <OrganizationIdentityCard
            portfolioMode={portfolioMode}
            switcherOpen={open}
            onToggleSwitcher={() => setOpen((v) => !v)}
          />
          {switcherDropdown}
        </div>
      </div>
  ...
  return (
    <div className="relative mb-2">
      <OrganizationIdentityCard portfolioMode={false} />
    </div>
  );
}
