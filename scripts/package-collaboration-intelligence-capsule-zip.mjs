#!/usr/bin/env node
/**
 * Package Collaboration Intelligence Capsule™ v1.0 — validation + stable latest.zip.
 * Institutional collaboration memory — HOW Founder and AI built Studio OS together.
 */
...
const CAPSULE_FOLDER = 'collaboration-intelligence';
const LATEST_ALIAS = 'latest.zip';
const RELEASE_MANIFEST = 'release.json';
const METADATA_FILE = 'collaboration-intelligence.json';
const DOWNLOAD_BASE = '/downloads/collaboration-intelligence-capsules';
const ARCHIVE_SUBDIR = 'archive';
const PERMANENT_LATEST_PATH = '/collaboration-intelligence/latest';
...
function validate(capsuleRoot, version) {
  ...
  for (const doc of READING_ORDER.filter((f) => f !== 'COLLABORATION_VALIDATION.md')) {
    if (!presentSet.has(doc)) errors.push(`Reading order missing: ${doc}`);
  }

  const orphans = presentMd.filter(
    (f) => !REQUIRED_FILES.includes(f) && f !== 'COLLABORATION_VALIDATION.md',
  );
  ...
  const inventory = new Set([...REQUIRED_FILES, 'COLLABORATION_VALIDATION.md']);
  for (const doc of REQUIRED_FILES) {
    const content = fs.readFileSync(path.join(capsuleRoot, doc), 'utf8');
    for (const link of extractInternalLinks(content)) {
      if (link.includes('/') || link.includes('StudioOS_')) continue;
      if (link === 'COLLABORATION_VALIDATION.md') continue;
  ...
  if (errors.length) {
    console.error('\n❌ Collaboration Intelligence Capsule validation failed:\n');
...
function writeCollaborationValidation(capsuleRoot, version, generatedAt, gitCommit, checksum, presentMd) {
  const body = `# Collaboration Intelligence Validation — Auto-Generated
...
`;
  fs.writeFileSync(path.join(capsuleRoot, 'COLLABORATION_VALIDATION.md'), body);
}

function writeMetadata(capsuleRoot, version, generatedAt) {
  const payload = {
    schemaVersion: 1,
    capsuleType: 'collaboration-intelligence',
    ...
    companionCapsules: {
      ...
      founderIntelligence: { version: '1.0.0', permanentUrl: '/founder-intelligence/latest' },
...
function migrateVersionedZipsToArchive(publicOut, archiveOut) {
  ...
  for (const f of fs.readdirSync(publicOut)) {
    if (!/^Collaboration_Intelligence_Capsule_v\d+\.\d+\.\d+\.zip$/.test(f)) continue;
...
function packageCapsule() {
  ...
  const fileName = `Collaboration_Intelligence_Capsule_v${version}.zip`;
  ...
  writeCollaborationValidation(dir, version, generatedAt, gitCommit, orderChecksum, presentMd);

  const publicOut = path.join(ROOT, 'public/downloads/collaboration-intelligence-capsules');
  const archiveOut = path.join(publicOut, ARCHIVE_SUBDIR);
  const releasesOut = path.join(ROOT, 'releases/downloads/collaboration-intelligence-capsules');
  const ciPublicDir = path.join(ROOT, 'public/collaboration-intelligence');
  fs.mkdirSync(publicOut, { recursive: true });
  fs.mkdirSync(archiveOut, { recursive: true });
  fs.mkdirSync(releasesOut, { recursive: true });
  fs.mkdirSync(ciPublicDir, { recursive: true });
  ...
  writeJson(path.join(ciPublicDir, RELEASE_MANIFEST), releaseManifest);
  ...
  writeJson(path.join(ROOT, 'api/_lib/collaboration-intelligence-capsule-build-manifest.json'), apiBuildManifest);
  writeJson(path.join(ROOT, 'api/_lib/collaboration-intelligence-capsule-release.json'), releaseManifest);

  console.log(`\nCollaboration Intelligence Capsule™ packaged:`);
  ...
  console.log(`  Download hub:      /collaboration-intelligence\n`);
}
