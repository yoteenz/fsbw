/**
 * Remove baked checkerboard from PSA avatar / nudge PNGs (conservative — character-safe).
 *
 * Fal/Ideogram exports often embed gray/white checker squares as opaque pixels instead
 * of true alpha. Edge flood + neutral-square cleanup only — no broad color heuristics.
 *
 * Usage:
 *   node scripts/psa-remove-checkerboard-alpha.mjs
 *   node scripts/psa-remove-checkerboard-alpha.mjs public/assets/psa-nudge-thought-bubble.png
 */
...
/** Neutral light/dark checker squares (fake transparency exports). */
...
async function flattenFile(filePath) {
  ...
  /** Interior checker islands (neutral squares not reached from the border). */
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const p = rgbaFromInt(img.getPixelColor(x, y));
      if (!isCheckerPixel(p)) continue;
  ...
  return { filename: path.basename(filePath), replaced, total: w * h };
}

const cliPaths = process.argv.slice(2).filter((a) => !a.startsWith('-'));
const files =
  cliPaths.length > 0
    ? cliPaths.map((p) => path.resolve(p))
    : fs
        .readdirSync(ASSETS_DIR)
        .filter(
          (f) =>
            (f.startsWith('psa-avatar-') && f.endsWith('.png')) ||
            f === 'psa-nudge-thought-bubble.png'
        )
...
for (const filePath of files) {
  const r = await flattenFile(filePath);
  console.log(
